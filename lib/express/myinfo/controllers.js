    ...
    const lookupPerson = (allowedAttributes) => async (req, res) => {
      ...
      if (disallowedAttributes.length > 0) {
        ...
        const transformPersona = encryptMyInfo
          ? encryptPersona
          : (person) => person
        let persona = assertions.myinfo[version].personas[req.params.uinfin]
        if (persona && persona.dynamic_age !== undefined) {
          const d = new Date()
          d.setFullYear(d.getFullYear() - persona.dynamic_age)
          const computedDob = d.toISOString().split('T')[0]
          persona = {
            ...persona,
            dob: {
