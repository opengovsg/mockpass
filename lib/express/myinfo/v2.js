...
function config(app, { serviceProvider }) {
  ...
  const lookupPerson = (allowedAttributes) => (req, res) => {
    ...
    if (disallowedAttributes.length > 0) {
      ...
      const persona = assertions.myinfo.v2.personas[req.params.uinfin]
  ...
  const allowedAttributes = assertions.myinfo.v2.attributes

  app.get(
    '/myinfo/v2/person-basic/:uinfin/',
    (req, res, next) => {
      ...
      authHeaderFields.url = `${req.protocol}://${req.get('host')}${
        req.baseUrl
      }${req.path}`
  ...
  app.get('/myinfo/v2/person/:uinfin/', (req, res) => {
  ...
  app.get('/myinfo/v2/authorise', consent.initAuthorization)
