...
const { sendUserInfoError } = require('./userinfo-error.js')
...
function config(app) {
  ...
  app.get(`${fapiRoutesBasePath}/userinfo`, async (req, res) => {
    try {
      const userInfo = await service.handleUserInfoRequest(req)
      return res.status(200).type('application/jwt').send(userInfo)
    } catch (e) {
      return sendUserInfoError(res, e)
    }
  })
