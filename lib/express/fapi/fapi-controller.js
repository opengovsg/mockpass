...
function config(app) {
  ...
  // For simplicity, using in-memory session store.
  app.use(
    session({
  ...
  app.get(`${FAPI_PATH}/.well-known/openid-configuration`, (req, res) => {
    return res.send(FapiUtils.fapiOidcConfiguration)
  })

  app.post(`${FAPI_PATH}/par`, async (req, res) => {
    try {
      const parRequest = await service.handleParRequest(req)
      req.session.parRequest = parRequest
      return res.send({ request_uri: parRequest.request_uri, expires_in: 60 })
    } catch (e) {
  ...
  app.get(`${FAPI_PATH}/auth`, async (req, res) => {
    try {
      ...
      return res.send(
        service.generateLoginPage(profiles, 'singPass', authRequest, req),
      )
    } catch (e) {
      return res.status(400).send({ error: e.message })
    }
