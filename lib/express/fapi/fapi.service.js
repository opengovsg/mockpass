...
class FapiService {
  ...
  async handleTokenRequest(req) {
    ...
    const client_id = await verifyClientAssertion(
      req.body.client_assertion,
      req.body.client_assertion_type,
    )
...
function verifyAllowedScopes(scope) {
  ...
  for (const s of scopes) {
    if (!FapiUtils.fapiOidcConfiguration.scopes_supported.includes(s))
...
function verifyAuthenticationContext(
  ...
  //TODO: For testing - This parameter seems to fail when provided for some login apps
