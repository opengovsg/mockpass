...
class FapiService {
  async handleParRequest(req) {
    ...
    const [, dpopJkt] = await Promise.all([
      ...
      verifyDpop(
        req.headers['dpop'],
        null,
        FapiUtils.fapiOidcConfiguration.pushed_authorization_request_endpoint,
      ),
    ])
  ...
  async handleAuthorizationRequest(req) {
    const parRequest = req.session.parRequest
    if (!parRequest) throw new Error('No PAR request found in session')
    verifyAuthRequestBody(req, parRequest)
    const authRequest = {
      ...parRequest,
    }
    req.session.authRequest = authRequest
    req.session.parRequest = null
    return authRequest
  }
  ...
  async handleTokenRequest(req) {
    ...
    await Promise.all([
      ...
      verifyDpop(
        req.headers['dpop'],
        authRequest.dpopJkt,
        FapiUtils.fapiOidcConfiguration.token_endpoint,
      ),
    ])
  ...
  generateLoginPage(profiles, idp, authRequest, req) {
    ...
    return render(LOGIN_TEMPLATE, {
      ...
      customProfileConfig: {
        ...
        redirectURI: req.session.authRequest.redirect_uri,
        state: req.session.authRequest.state,
        nonce: req.session.authRequest.nonce,
      },
    })
  }
  ...
  async generateIdToken(req, authRequest) {
    ...
    // console.debug(profile) //useful to extract the code for debugging without a RP
    const id_token = {
      sub: profile.uuid,
      aud: req.client_id,
      ...
      sub_type: 'user',
      amr: ['pwd'],
      iss: FapiUtils.fapiOidcConfiguration.issuer,
...
async function verifyClientAssertion(clientAssertion, clientAssertionType) {
  assert(clientAssertion, 'Client assertion is required')
  assert(clientAssertionType, 'Client assertion type is required')
  if (
    clientAssertionType !==
  ...
  const { payload } = await jose
    .jwtVerify(clientAssertion, keyToTry, {
      algorithms: ['ES256'],
      clockTolerance: FapiUtils.dpopConfiguration.DPOP_CLOCK_SKEW,
  ...
  if (payload.aud !== FapiUtils.fapiOidcConfiguration.issuer)
...
function verifyAllowedScopes(scope) {
  ...
  if (!FapiUtils.fapiOidcConfiguration.scopes_supported.includes(scope))
...
function verifyResponseType(responseType) {
  ...
  if (
    !FapiUtils.fapiOidcConfiguration.response_types_supported.includes(
      responseType,
    )
  )
...
function verifyAcrValues(acrValue) {
  assert(acrValue, 'acr values is required')
...
function verifyAuthenticationContext(
  ...
  assert(authenticationContextType, 'Authentication context type is required')
  if (
