...
class FapiService {
  ...
  async handleTokenRequest(req) {
    ...
    const { authRequest, profile } = authCodeSession
    ...
    const dpopJkt = await verifyDpop(
      req.headers['dpop'],
      authRequest.dpopJkt,
      tokenEndpoint,
    )
    return this.generateTokenResponse(req, { profile, authRequest, dpopJkt })
  }
  ...
  async generateTokenResponse(req, { profile, authRequest, dpopJkt }) {
    ...
    return {
      access_token: generateAccessTokenForFapi({
        profile,
        authRequest,
        dpopJkt,
      }),
      id_token: await signAndEncrypt(id_token),
      token_type: 'DPoP',
      expires_in: FAPI_ACCESS_TOKEN_EXPIRY_SECONDS,
    }
  }

  async handleUserInfoRequest(req) {
    const accessToken = extractDpopAccessToken(req)
    const session = lookUpByAccessToken(accessToken)
    if (!session) throw invalidToken('Access token is invalid or has expired')

    const { profile, authRequest, dpopJkt } = session
    const { issuer, userinfo_endpoint } =
      FapiUtils.getFapiOpenIdConfiguration(req)

    try {
      await verifyDpop(req.headers['dpop'], dpopJkt, userinfo_endpoint, {
        expectedMethod: 'GET',
        accessToken,
      })
    } catch (error) {
      throw invalidDpopProof(error.message)
    }

    const userInfo = {
      person_info: buildPersonInfo(profile, authRequest.scope),
      iss: issuer,
      sub: profile.uuid,
      aud: authRequest.client_id,
      iat: Math.floor(Date.now() / 1000),
    ...
    // An unreachable or keyless client JWKS is the caller's problem; a missing
    // ASP signing key is ours, and falls through to a 500.
    const signedUserInfo = await signWithAspSigningKey(userInfo)
    try {
      return await encryptForClient(signedUserInfo)
    } catch (error) {
...
async function signAndEncrypt(payload) {
  return encryptForClient(await signWithAspSigningKey(payload))
}

async function signWithAspSigningKey(payload) {
...
async function encryptForClient(signedJwt) {
  let rpEncPublicKey
  ...
  return new jose.CompactEncrypt(new TextEncoder().encode(signedJwt))
    .setProtectedHeader(encryptionProtectedHeader)
    .encrypt(encKey)
}

function extractDpopAccessToken(req) {
  const authorization = req.get('authorization')
  if (!authorization) throw invalidToken('Missing Authorization header')

  const [scheme, token] = authorization.trim().split(' ')
  if (scheme !== 'DPoP' || !token)
    throw invalidToken('Authorization header must use the DPoP scheme')

  return token
}

// Only MyInfo persona profiles carry claims. The rest can answer for the uinfin
// they were created with, and nothing else.
function lookUpPersonAttribute(profile, attribute) {
  const claim = profile.claims && profile.claims[attribute]
  if (claim) return claim
  if (attribute === 'uinfin') return makeClaim(profile.nric)
  return makeUnavailableClaim()
}

function buildPersonInfo(profile, scope) {
  const requestedAttributes = scope
    .split(' ')
    .filter((requested) => FapiUtils.MyInfoPersonScopes.includes(requested))

  const personInfo = {}
  for (const attribute of requestedAttributes) {
    personInfo[attribute] = lookUpPersonAttribute(profile, attribute)
  }
  return personInfo
}
...
async function verifyDpop(
  ...
  dpop_jkt,
  expectedEndpoint,
  { expectedMethod = 'POST', accessToken = null } = {},
) {
  ...
  if (dpop) {
    ...
    const requiredClaims = ['jti', 'iat', 'htu', 'htm']
    if (accessToken) requiredClaims.push('ath')

    let payload
    try {
      const key = await jose.importJWK(jwk, 'ES256')
      const verified = await jose.jwtVerify(dpopToken, key, {
    ...
    if (htm.toUpperCase() !== expectedMethod) {
      throw new Error(`DPoP htm must be ${expectedMethod} for this endpoint`)
    }
    if (accessToken && ath !== FapiUtils.hashAccessToken(accessToken)) {
...
function verifyAllowedScopes(scope) {
  ...
  for (const s of scopes) {
    if (!FapiUtils.SupportedScopes.includes(s))
