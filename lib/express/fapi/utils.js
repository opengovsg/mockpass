...
const MyInfoPersonScopes = assertions.myinfo.v3.attributes.basic
const SupportedScopes = ['openid', 'user.identity', ...MyInfoPersonScopes]
...
async function generateDpopToken(req, publicKey, privateKey) {
  const { access_token: accessToken, method = 'POST' } = req.body
  const dpop = {
    ...
    body: {
      ...
      nonce: randomUUID(),
      ...(accessToken ? { ath: hashAccessToken(accessToken) } : {}),
    },
  }
