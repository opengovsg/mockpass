...
function config(app, { showLoginPage, serviceProvider, isStateless }) {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // Change sub from `s=${nric},u=${password}`
        // to `u=${password}` to be consistent with userinfo sub
  ...
  app.get(`${PATH_PREFIX}/userinfo`, async (req, res) => {
    ...
    const nric = assertions.oidc.singPass.find((p) => p.password === password).nric
...
const sgIDScopeToMyInfoField = (persona, scope) => {
  switch (scope) {
    ...
    case 'openid':
      return defaultUndefinedToNA(persona.password?.value)
    case 'myinfo.name':
