...
function config(app, { showLoginPage, serviceProvider, isStateless }) {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const privateKey = await jose.importPKCS8(signingPem.toString())
  ...
  app.get(`${PATH_PREFIX}/userinfo`, async (req, res) => {
    ...
    const rawOct = crypto.randomBytes(32)
    const payloadKeyJwk = {
      kty: 'oct',
      k: rawOct.toString('base64url'),
      alg: 'A256GCM',
    }
    const payloadKey = await jose.importJWK(payloadKeyJwk, 'A256GCM')

    const encryptPayload = async (field) => {
      return await new jose.CompactEncrypt(Buffer.from(field))
    ...
    const encryptionKey = await jose.importSPKI(
      serviceProvider.pubKey.toString(),
      'RSA-OAEP',
    )

    const plaintextPayloadKey = JSON.stringify(payloadKeyJwk)
    const encryptedPayloadKey = await new jose.CompactEncrypt(
      Buffer.from(plaintextPayloadKey),
    )
  ...
  app.get(`${VERSION_PREFIX}/.well-known/jwks.json`, async (_req, res) => {
    const publicKey = crypto.createPublicKey(signingPem)
    const spkiPem = publicKey.export({ type: 'spki', format: 'pem' })
    const key = await jose.importSPKI(spkiPem, 'RS256')
