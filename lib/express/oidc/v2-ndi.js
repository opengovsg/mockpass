...
const aspPublic = fs.readFileSync(
  path.resolve(__dirname, '../../../static/certs/spcp-v2-asp-public.json'),
)

const aspSecret = fs.readFileSync(
  path.resolve(__dirname, '../../../static/certs/spcp-v2-asp-secret.json'),
)

const rpPublic = fs.readFileSync(
  path.resolve(__dirname, '../../../static/certs/spcp-v2-rp-public.json'),
)
  ...
  for (const idp of ['singPass', 'corpPass']) {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        const clientJwksEndpoint =
          idp === 'singPass'
            ? process.env.SP_RP_JWKS_ENDPOINT
            : process.env.CP_RP_JWKS_ENDPOINT

        const rpKeysetString = clientJwksEndpoint
          ? await fetch(clientJwksEndpoint, {
              method: 'GET',
        ...
        try {
          ...
          return res.status(400).send('Unable to parse RP keyset')
        }
        ...
        try {
          ...
          return res.status(400).send('Unable to verify client_assertion')
        }
        ...
        try {
          ...
          return res.status(400).send('Unable to parse client_assertion')
        }
    ...
    app.get(
      ...
      (req, res) => {
        ...
        const data = {
          ...
          token_endpoint_auth_signing_alg_values_supported: ['ES512'], // omits ES256 and ES384 (SP)
          id_token_signing_alg_values_supported: ['ES256'],
          id_token_encryption_alg_values_supported: ['ECDH-ES+A256KW'], // omits ECDH-ES+A192KW, ECDH-ES+A128KW and RSA-OAEP-256 (SP)
        ...
        if (idp === 'corpPass') {
          ...
          // Omit authorization-info_endpoint
        }
