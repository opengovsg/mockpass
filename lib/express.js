...
const LOGIN_TEMPLATE = fs.readFileSync(path.resolve(__dirname, './Login_page.html'), 'utf8')
var assets = {}
assets.identity = []
      ...
      if (showLoginPage) {
        ...
        const someValues = identities
          .map((value, index) => {
            ...
            assets.identity[index] = {'id': `${value}`, 'URL': `${assertURL}`}
          })
        const response = render(LOGIN_TEMPLATE, assets.identity)
        res.send(response)
      } else {
